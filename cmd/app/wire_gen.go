...
func initSystem(ctx context.Context, config2 *types.Config) (*server.System, error) {
	...
	k8sManager := manager.ProvideK8sManager()
	managerFactory := manager.ProvideManagerFactory(k8sManager)
	controller := instance.ProvideController(transactor, instanceStore, serverStore, principalStore, service, managerFactory)
	...
	applicationController := application.ProvideController(transactor, configService, schemaService, specService, applicationStore, serverController, variableController, gitpublicController, volumeController, triggererTriggerer, cancelerCanceler, managerFactory)
	...
	runnerManager := manager2.ProvideRunnerManager(schedulerScheduler, serverStore, applicationStore, deploymentStore, volumeStore, logStore, variableController, configService, githubappService, streamer, logStream)
	runnerClient := client.ProvideRunnerClient(runnerManager)
	agentAgent := agent.ProvideAgent(runnerClient)
