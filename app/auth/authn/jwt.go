...
func (a *JWTAuthenticator) Authenticate(r *http.Request) (*auth.Session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("JWT: parsing of JWT claims failed: %w", err)
	}

	if !parsed.Valid {
		return nil, errors.New("JWT: parsed JWT token is invalid")
	}

	if _, ok := parsed.Method.(*gojwt.SigningMethodHMAC); !ok {
		return nil, errors.New("JWT: invalid HMAC signature for JWT")
	}
