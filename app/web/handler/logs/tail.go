...
func HandleTailLogs(appCtx context.Context, logCtrl *logs.Controller) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		deployment, _ := request.DeploymentFrom(ctx)

		linec, errc, err := logCtrl.Tail(ctx, deployment.ID)
		if err != nil {
		...
		render.StreamSSE(ctx, w, appCtx.Done(), linec, errc, renderInit, renderFunc)
	}
}
...
func renderFunc(ctx context.Context, w io.Writer, line *types.LogLine) error {
	if line.Message == "" {
		return nil
	}
	...
	if err := vlog.LogLine(line).Render(ctx, writer); err != nil {
		log.Ctx(ctx).Err(err).Msg("sse render: error rendering log line")
		return err
	}
