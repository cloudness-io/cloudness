CREATE TABLE jobs (
    ...
    job_group_id TEXT NOT NULL DEFAULT ''
);

CREATE INDEX jobs_scheduled ON jobs (job_scheduled)
WHERE
    job_state = 'scheduled';

CREATE INDEX jobs_run_deadline ON jobs (job_run_deadline)
WHERE
    job_state = 'running';

CREATE INDEX jobs_last_executed ON jobs (job_last_executed)
WHERE
    job_is_recurring = FALSE
    AND (
        job_state = 'finished'
        OR job_state = 'failed'
        OR job_state = 'canceled'
    );
