...
if ! kubectl rollout status deployment/$DEPLOYMENT_NAME -n "$NAMESPACE" --timeout=2m; then # Add a timeout for the rollout itself
  log "ERROR" "Deployment rollout did not succeed within rollout time. Rolling back..."
...
# 7- Cleaning up Statefulsets if any
kubectl delete statefulset/$DEPLOYMENT_NAME -n "$NAMESPACE" --ignore-not-found=true 
