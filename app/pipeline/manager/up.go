...
func (m *runnerManager) up(ctx context.Context, deployment *types.Deployment) error {
	log := log.With().
	...
	// update application
	if app.DeploymentStatus != enum.ApplicationDeploymentStatusDeploying {
		app.DeploymentStatus = enum.ApplicationDeploymentStatusDeploying
